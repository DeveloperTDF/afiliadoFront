...
export class Tab2Page implements OnInit {
  ...
  ngOnInit() {
    this.activateroute.params
      .pipe(switchMap(({ id }) => this.afiliadoservice.getAfiliadoPorId(id)))
  ...
  onSubmit() {
    ...
    if (this.afiliado.id) {
      this.afiliadoservice
        .actualizarAfiliado(this.afiliado)
        .subscribe((afiliado) => console.log('actualizando', afiliado));
      this.router.navigate(['tabs/tab1']);
    } else {
      this.afiliadoservice.agregarAfiliado(this.afiliado).subscribe((resp) => {
  ...
  borrar() {
    if (!this.afiliado.id) {
      ...
      this.afiliadoservice.eliminarAfiliado(this.afiliado.id!).subscribe(
        (resp) => {
